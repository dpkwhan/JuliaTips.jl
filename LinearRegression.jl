...
begin
	...
	using StatsModels, Tables, RDatasets, StatsPlots, GLM
end
...
# ╔═╡ 31b9cb1d-cf7e-4a21-b3b7-d328d599b11d
formaldehyde = dataset("datasets", "Formaldehyde");

# ╔═╡ 6006bde4-9a29-4e06-89ec-50309d61998d
formaldehyde

# ╔═╡ 7d08dc77-8e57-4848-a4d9-eff581b59086
@df formaldehyde scatter(
    :Carb,
    :OptDen,
    xlabel = "Carbohydrate Level",
...
Creating model matrices from a data table can be a tedious and error-prone operation.  In addition, _statistical inference_ regarding a linear model often considers groups of columns generated by model _terms_.  The [`GLM.jl`](https://github.com/JuliaStats/GLM.jl) package provides methods to fit and analyze linear models and generalized linear models (described later) using a _formula/data_ specification similar to that in _R_.
"""

# ╔═╡ b34cee7c-881f-487c-961b-24ba13c260a5
m = fit(LinearModel, @formula(OptDen ~ 1 + Carb), formaldehyde);

# ╔═╡ f3e48632-0c70-4a29-aa96-3a77b5f0fc5f
md"""
The evaluation of the formula is performed by the [`StatsModels`](https://github.com/JuliaStats/StatsModels.jl) package in stages.
"""

# ╔═╡ dbe020f3-2258-4a59-8af8-1a392782db16
let
	f = @formula(OptDen ~ 1 + Carb)
	y, X = modelcols(apply_schema(f, schema(formaldehyde)), formaldehyde)
end;

# ╔═╡ ce0bc481-5346-4d77-837a-607b96625a5c
β₂ = coef(m)
...
# ╔═╡ ab133b37-fcb4-42af-9b39-5b3e636be432
md"""
...
In fact, when $p>1$ there are several.  A specific choice of $\mathbf{A}$ is an upper triangular matrix with positive elements on the diagonal, usually written as $\mathbf{R}$ and called the upper Cholesky factor of $\mathbf{X^\prime X}$.

Let's see the following code in Julia:
"""

# ╔═╡ 5fd15678-17de-4eb9-8f34-60b2b14e6d69
begin
	xpx = X'X
	ch = cholesky(xpx)
	ch.U'ch.U ≈ xpx
...
# ╔═╡ 4ced5f6a-6f19-4f7e-9337-d33b3cf573db
md"""
...
# ╔═╡ 32ceea2a-d52f-49cc-bbe2-73194029a427
v = ldiv!(ch.U', X'y);

# ╔═╡ 8d59e018-2ec3-49ee-8cf1-2b2d7cc33387
md"""
...
# ╔═╡ 80e7c3a6-dfad-495b-a96d-e53448947006
β₃ = ldiv!(ch.U, copy(v))  # solution from the Cholesky factorization

# ╔═╡ ae135cb8-f740-400d-b0e9-d32d31fa2fd3
β₃ ≈ β₁

# ╔═╡ ef8792c3-0466-4d42-9d12-18ce65dae490
md"""
These steps are combined in one of the many `LinearAlgebra` methods for solutions of equations.
"""

# ╔═╡ 8379d4ec-30e0-4070-8a9a-fc0ea2f22dad
β₄ = ldiv!(ch, X'y)
...
# ╔═╡ a55267da-ec13-461f-9155-c63d4365d866
begin
...
# ╔═╡ c4971eee-5e30-457f-b97a-8af11db50ab6
md"The least squares solution from the augmented Cholesky:"

# ╔═╡ c6de7e74-2a89-44f1-890e-6e59606078f6
β₅ = ldiv!(RXX, copy(rXy))

# ╔═╡ ff7e17e9-3f0d-4d99-b2b2-41db80afbd15
β₅ ≈ β₁

# ╔═╡ 65218459-5f8a-4ebf-b0a1-f69afe21f4c6
md"Check on residual sum of squares:"

# ╔═╡ d4e051a6-5bcf-439e-86bc-da85518251ac
abs2(cha.U[end,end]) ≈ sum(abs2, y - X*β)

# ╔═╡ d25a47c3-6ffe-43e0-9ab4-602df43ea4e9
md"""
One reason for writing the least squares solution in this way is that we will use a similar decomposition for linear mixed models later.  Another is that when dealing with very large data sets we may wish to parallelize the calculation over many cores or over many processors.  The natural way to parallelize the calculation is in blocks of rows and the augmented Cholesky can be formed row by row using a [`lowrankupdate`](https://docs.julialang.org/en/v1/stdlib/LinearAlgebra/#LinearAlgebra.lowrankupdate).
"""
...
# ╔═╡ a4aa4a61-e334-4d35-8335-a0ecc920ec6d
formrows = Tables.rowtable(formaldehyde);

# ╔═╡ f698e807-581c-4314-a33e-e71a44bd6996
md"""
then initialize a Cholesky factor and zero out its contents
"""

# ╔═╡ 02710aee-6b7b-4f06-a4c9-bb44ebd2a79c
begin
	chr = cholesky(zeros(3, 3) + I)  # initialize
	fill!(chr.factors, 0)            # zero out the contents
end;

# ╔═╡ db5a016d-4c91-4fa7-8035-57b2f29b546c
md"""
Update by rows
"""

# ╔═╡ 6cca8455-6aa6-4074-9fbb-2e5201edb09f
begin
	fill!(chr.factors, 0)
	for r in formrows
...
# ╔═╡ 6c87547b-1e21-4e34-8a63-10c84c7d841c
md"""
...
# ╔═╡ 655b2451-8644-4129-84cd-7f4ee876d0f7
qrX.R'qrX.R;

# ╔═╡ 896b894d-c10a-46e9-bc4e-d26af34bd0e5
md"""
That is,
"""

# ╔═╡ d4d758b3-6fe8-4683-9570-1e601258efe1
qrX.R'qrX.R ≈ xpx

# ╔═╡ cba6aa02-cb61-4803-9125-b31f7c272149
md"""
